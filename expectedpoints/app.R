  ...
  {dashboardSidebar(width = 250,
                   ...
                   sidebarMenu(
                     ...
                     selectizeInput("selectTeam",
                                 "Select Team:",
                                 choices = c("All", as.character(sort(unique(epdata$posteam)))),
                                 selected = "KC"),
...
server <- shinyServer(function(input, output, session) {
  ...
  allCols <- reactive({
    epdata %>%
      {if (input$weeklyRadio == "Weekly") group_by(., gsis_name, gsis_pos, posteam, week) else group_by(., gsis_name, gsis_pos, posteam)} %>%
  ...
  selCols <- reactive({
    filter2() %>%
      {if (input$selectCol == "Exp Points" & input$weeklyRadio == "Weekly")
        dplyr::select(., week, gsis_name, posteam, gsis_pos, Games, eRecFP, RecFP, RecDiff, eRushFP, RushFP, RushDiff, eFP, FP, Diff, `eFP/G`, `FP/G`, `Diff/G`) %>% arrange(desc(`eFP/G`))
        else if (input$selectCol == "Rush Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, posteam, gsis_pos, Games, Rushes, eRushYD, RushYD, RushYDDiff, eTDRush, RushTD, RushTDDiff, eRush1D, Rush1D, Rush1DDiff) %>% arrange(desc(RushYD))
        else if (input$selectCol == "Rec Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, posteam, gsis_pos, Games, Targets, Catches, eRecYD, RecYD, RecYDDiff, eTDRec, RecTD, RecTDDiff, eRec1D, Rec1D, Rec1DDiff) %>% arrange(desc(RecYD))
        else if (input$selectCol == "Total Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, posteam, gsis_pos, Games, eYD, YDs, YDDiff, eTD, TDs, TDDiff, e1D, total1Ds, total1Ddiff) %>% arrange(desc(YDs))
        
        else if (input$selectCol == "Rate Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, posteam, gsis_pos, AYshare, TargetShare, WOPR, RACR, YPTPA, eFPshare, FPshare) %>% arrange(desc(eFPshare))
        else if (input$selectCol == "Exp Points")
          dplyr::select(., gsis_name, posteam, gsis_pos, Games, eRecFP, RecFP, RecDiff, eRushFP, RushFP, RushDiff, eFP, FP, Diff, `eFP/G`, `FP/G`, `Diff/G`) %>% arrange(desc(`eFP/G`))
        else if (input$selectCol == "Rush Stats")
          dplyr::select(., gsis_name, posteam, gsis_pos, Games, Rushes, eRushYD, RushYD, RushYDDiff, eTDRush, RushTD, RushTDDiff, eRush1D, Rush1D, Rush1DDiff) %>% arrange(desc(RushYD))
        else if (input$selectCol == "Rec Stats")
          dplyr::select(., gsis_name, posteam, gsis_pos, Games, Targets, Catches, eRecYD, RecYD, RecYDDiff, eTDRec, RecTD, RecTDDiff, eRec1D, Rec1D, Rec1DDiff) %>% arrange(desc(RecYD))   
        else if (input$selectCol == "Total Stats")
          dplyr::select(., gsis_name, posteam, gsis_pos, Games, eYD, YDs, YDDiff, eTD, TDs, TDDiff, e1D, total1Ds, total1Ddiff) %>% arrange(desc(YDs))        
        else if (input$selectCol == "Rate Stats")
          dplyr::select(., gsis_name, posteam, gsis_pos, Games, AYshare, TargetShare, WOPR, RACR, YPTPA, eFPshare, FPshare) %>% arrange(desc(eFPshare))
      }
