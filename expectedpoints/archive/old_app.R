  ...
  {dashboardSidebar(width = 250,
                   ...
                   sidebarMenu(
                     ...
                     selectizeInput("selectTeam",
                                 "Select Team:",
                                 choices = c("All", as.character(sort(unique(epdata$team)))),
                                 selected = "KC"),
                     ...
                     conditionalPanel(condition = "input.weeklyRadio == 'Weekly'",
                                      selectizeInput("selectWeeks",
                                                     "Select Weeks:",
                                                     choices = c("All", sort(unique(epdata$week))),
                                                     selected = "All",
                                                     multiple = TRUE)),
                     conditionalPanel(condition = "input.weeklyRadio == 'Cumulative'",
                                      sliderTextInput("selectRange",
                                                      "Select Range:",
                                                      choices = c("All", sort(unique(epdata$week))),
                                                      selected = c("All", sort(unique(epdata$week)))[c(4,8)])),
...
server <- shinyServer(function(input, output, session) {
  ...
  allCols <- reactive({
    epdata %>%
      {if (input$weeklyRadio == "Weekly") group_by(., gsis_name, gsis_pos, team, week) else group_by(., gsis_name, gsis_pos, team)} %>%
  ...
  filter1 <- reactive({
    allCols() %>%
      {if (input$selectTeam != "All") filter(., team == input$selectTeam) else . } %>%
      {if (input$selectPos  != "All") filter(., gsis_pos == input$selectPos) else .} %>%
      {if (input$weeklyRadio == "Weekly" & input$selectWeeks != "All") filter(., week %in% input$selectWeeks) else .} 
  })
  
  filter2 <- reactive({
    filter1() %>%
      {if (input$selectPlayers != "All") filter(., gsis_name %in% input$selectPlayers) else .}
  })
  
  selCols <- reactive({
    filter2() %>%
      {if (input$selectCol == "Exp Points" & input$weeklyRadio == "Weekly")
        dplyr::select(., week, gsis_name, team, gsis_pos, Games, eRecFP, RecFP, RecDiff, eRushFP, RushFP, RushDiff, eFP, FP, Diff, `eFP/G`, `FP/G`, `Diff/G`) %>% arrange(desc(`eFP/G`))
        else if (input$selectCol == "Rush Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, team, gsis_pos, Games, Rushes, eRushYD, RushYD, RushYDDiff, eTDRush, RushTD, RushTDDiff, eRush1D, Rush1D, Rush1DDiff) %>% arrange(desc(RushYD))
        else if (input$selectCol == "Rec Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, team, gsis_pos, Games, Targets, Catches, eRecYD, RecYD, RecYDDiff, eTDRec, RecTD, RecTDDiff, eRec1D, Rec1D, Rec1DDiff) %>% arrange(desc(RecYD))
        else if (input$selectCol == "Total Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, team, gsis_pos, Games, eYD, YDs, YDDiff, eTD, TDs, TDDiff, e1D, total1Ds, total1Ddiff) %>% arrange(desc(YDs))
        
        else if (input$selectCol == "Rate Stats" & input$weeklyRadio == "Weekly")
          dplyr::select(., week, gsis_name, team, gsis_pos, AYshare, TargetShare, WOPR, RACR, YPTPA, eFPshare, FPshare) %>% arrange(desc(eFPshare))
        else if (input$selectCol == "Exp Points")
          dplyr::select(., gsis_name, team, gsis_pos, Games, eRecFP, RecFP, RecDiff, eRushFP, RushFP, RushDiff, eFP, FP, Diff, `eFP/G`, `FP/G`, `Diff/G`) %>% arrange(desc(`eFP/G`))
        else if (input$selectCol == "Rush Stats")
          dplyr::select(., gsis_name, team, gsis_pos, Games, Rushes, eRushYD, RushYD, RushYDDiff, eTDRush, RushTD, RushTDDiff, eRush1D, Rush1D, Rush1DDiff) %>% arrange(desc(RushYD))
        else if (input$selectCol == "Rec Stats")
          dplyr::select(., gsis_name, team, gsis_pos, Games, Targets, Catches, eRecYD, RecYD, RecYDDiff, eTDRec, RecTD, RecTDDiff, eRec1D, Rec1D, Rec1DDiff) %>% arrange(desc(RecYD))   
        else if (input$selectCol == "Total Stats")
          dplyr::select(., gsis_name, team, gsis_pos, Games, eYD, YDs, YDDiff, eTD, TDs, TDDiff, e1D, total1Ds, total1Ddiff) %>% arrange(desc(YDs))        
        else if (input$selectCol == "Rate Stats")
          dplyr::select(., gsis_name, team, gsis_pos, Games, AYshare, TargetShare, WOPR, RACR, YPTPA, eFPshare, FPshare) %>% arrange(desc(eFPshare))
      }
  ...
  weekPivot <- reactive({
    filter2() %>%
      select(gsis_name, week, input$selectVar) %>%
      arrange(week) %>%
  ...
  observeEvent({input$selectTeam
    ...
    input$selectPlayers},{
      ...
      updateSelectizeInput(session, 'selectPlayers',
                           choices = c("All", as.character(sort(unique(filter1()$gsis_name)))),
...
#    plotdf<-plotdf #%>% 
    ...
    ggplot(plotdf,
           aes_string(x = plotdf$week, y = input$selectVar, color = plotdf$gsis_name)) +
      geom_point(size = 3) +
      theme_bw() + 
